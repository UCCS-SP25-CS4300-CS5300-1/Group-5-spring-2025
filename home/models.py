...
class Facility(models.Model):
    ...
    # RIDB api returns accessbility text..
    # can later add logic to make this true or false by detecting
    ...
    # commenting out & may get rid of reservation url,
    # API is kind of useless w/ returning the url, its basically
...
class TripDetails(models.Model):
    ...
    packing_list = models.TextField(
        blank=True, help_text="Comma-separated packing list items generated via AI."
    )
...
class CampUser(AbstractUser):
    ...
    user_permissions = models.ManyToManyField(
        "auth.Permission", related_name="campuser_set", blank=True
    )

    # this line of code is unneeded
    # but removing it breaks a lot of tests, need to redesign tests when get the chance
